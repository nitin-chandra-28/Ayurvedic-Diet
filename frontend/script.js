...
// ===============================
// Dynamic Theme from Image Colors
// ===============================
...
function adjustHsl(hex, { dh = 0, ds = 0, dl = 0 }) {
  const { r, g, b } = /^#/.test(hex) ? hexToRgb(hex) : hex;
  const hsl = rgbToHsl(r, g, b);
  let h = (hsl.h + dh + 360) % 360;
  let s = Math.min(100, Math.max(0, hsl.s + ds));
  let l = Math.min(100, Math.max(0, hsl.l + dl));
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        if ((rr + gg + bb) / 3 > 245 || (rr + gg + bb) / 3 < 10) continue;
        r += rr; g += gg; b += bb; count++;
      }
      if (count === 0) return resolve('#556B2F');
      r = Math.round(r / count); g = Math.round(g / count); b = Math.round(b / count);
      resolve(rgbToHex(r, g, b));
    };
    img.onerror = () => resolve('#556B2F');
    img.src = src;
  });
}
...
function applyTheme({ primary, secondary, accent }) {
  ...
  // Update hero overlay gradient to use new colors
  const hero = document.querySelector('[data-alt]');
  if (hero) {
    const bg = getComputedStyle(hero).backgroundImage;
    // Extract the image URL (last url(...))
    const urls = [...bg.matchAll(/url\((?:"|')?([^\)"']+)(?:"|')?\)/g)].map(m => m[1]);
    const imgUrl = urls[urls.length - 1];
    hero.style.backgroundImage = `linear-gradient(135deg, ${primary}E8 0%, ${accent}D9 100%), url("${imgUrl}")`;
    hero.style.backgroundBlendMode = 'overlay';
  }

  // Decorative blobs: override background colors inline
  const deco = document.querySelectorAll('.fixed.inset-0 .bg-primary, .fixed.inset-0 .bg-secondary, .fixed.inset-0 .bg-accent');
  deco.forEach((el, idx) => {
    el.style.background = [primary, secondary, accent][idx % 3];
  });
}

async function initThemeFromHeroImage() {
  try {
    const hero = document.querySelector('[data-alt]');
    if (!hero) return;
    const bg = getComputedStyle(hero).backgroundImage;
    ...
    const imgUrl = urls[urls.length - 1];
    if (!imgUrl) return;
    const primary = await extractDominantColorFromImage(imgUrl);
    const accent = adjustHsl(primary, { dh: 20, dl: 10 });
    const secondary = adjustHsl(primary, { dh: 40, dl: 15 });
    applyTheme({ primary, secondary, accent });
    const txt = bestTextColor(primary);
    document.body.style.setProperty('--theme-hero-text', txt);
    // also set rgb for alpha variants
    const t = hexToRgb(txt);
    document.body.style.setProperty('--theme-hero-text-rgb', `${t.r}, ${t.g}, ${t.b}`);
    log(`Theme updated from image: P=${primary} S=${secondary} A=${accent}`);
  } catch (e) {
    log('Theme init failed: ' + e.message, 'warn');
  }
}
...
async function setHeroBackgroundFromPreferredImage() {
  const hero = document.querySelector('[data-alt]');
  if (!hero) return;
  const candidates = ['helo.jpg', 'helo.jpj'];
  for (const src of candidates) {
    const ok = await testImage(src);
    if (ok) {
      const current = hero.style.backgroundImage;
      let gradient = 'linear-gradient(135deg, rgba(85, 107, 47, 0.92) 0%, rgba(143, 188, 143, 0.85) 100%)';
      if (current && current.includes('linear-gradient')) {
        const m = current.match(/linear-gradient\([^\)]*\)/);
        if (m && m[0]) gradient = m[0];
      }
      hero.style.backgroundImage = `${gradient}, url("${src}")`;
      hero.style.backgroundBlendMode = 'overlay';
      return;
    }
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  await setHeroBackgroundFromPreferredImage();
  initThemeFromHeroImage();
});
...
function translatePage() {
  ...
  document.querySelectorAll('[data-i18n-html]')
    .forEach(el => { el.innerHTML = $t(el.getAttribute('data-i18n-html')); });
}

function initI18n() {
  const defaultLng = localStorage.getItem('lang') || 'en';
  const resources = {
    en: {
      translation: {
        nav: { learn: 'Learn Ayurveda' },
        hero: {
          badge: '✨ Ancient Wisdom, Modern Science',
          title: 'Get Your Personalized<br/>Ayurvedic Diet Plan',
          subtitle: 'Balance your mind, body, and spirit with a diet tailored to your unique dosha constitution',
          explore: 'Explore Features'
        },
        how: { title: 'How It Works' },
        cta: { startJourney: 'Start Your Journey Now' },
        features: {
          prakriti: { title: 'My Prakriti', desc: 'Discover your unique mind-body constitution.' },
          generate: { title: 'Generate Plan', desc: 'Create a new diet plan based on your goals.' },
          plans: { title: 'My Diet Plans', desc: 'Access your saved and past diet plans.' },
          herbs: { title: 'Explore Herbs', desc: 'Learn about Ayurvedic herbs and their benefits.' },
          learn: { title: 'Learn Ayurveda', desc: 'Deepen your knowledge of Ayurvedic principles.' },
          advisory: { title: 'Smart Advisory', desc: 'Get AI-powered advice for your wellness journey.' }
        },
        footer: { copy: '© 2024 AyurBalance. All rights reserved.' }
      }
    },
    hi: {
      translation: {
        nav: { learn: 'आयुर्वेद सीखें' },
        hero: {
          badge: '✨ प्राचीन ज्ञान, आधुनिक विज्ञान',
          title: 'अपना व्यक्तिगत<br/>आयुर्वेदिक आहार योजना प्राप्त करें',
          subtitle: 'अपने अद्वितीय दोष constitution के अनुसार संतुलित आहार के साथ मन, शरीर और आत्मा का संतुलन बनाएँ',
          explore: 'विशेषताएँ देखें'
        },
        how: { title: 'यह कैसे काम करता है' },
        cta: { startJourney: 'अपनी यात्रा शुरू करें' },
        features: {
          prakriti: { title: 'मेरी प्रकृति', desc: 'अपनी अनोखी मन-शरीर संरचना जानें।' },
          generate: { title: 'योजना बनाएं', desc: 'अपने लक्ष्यों के आधार पर नई आहार योजना बनाएं।' },
          plans: { title: 'मेरी आहार योजनाएँ', desc: 'अपनी सहेजी हुई और पुरानी योजनाएँ देखें।' },
          herbs: { title: 'जड़ी-बूटियाँ जानें', desc: 'आयुर्वेदिक जड़ी-बूटियों और उनके लाभों के बारे में जानें।' },
          learn: { title: 'आयुर्वेद सीखें', desc: 'आयुर्वेदिक सिद्धांतों का ज्ञान बढ़ाएँ।' },
          advisory: { title: 'स्मार्ट सलाह', desc: 'आपकी स्वास्थ्य यात्रा के लिए AI आधारित सलाह पाएँ।' }
        },
        footer: { copy: '© 2024 AyurBalance. सर्वाधिकार सुरक्षित।' }
      }
    },
    mr: {
      translation: {
        nav: { learn: 'आयुर्वेद शिका' },
        hero: {
          badge: '✨ प्राचीन ज्ञान, आधुनिक विज्ञान',
          title: 'तुमची वैयक्तिक<br/>आयुर्वेदिक आहार योजना मिळवा',
          subtitle: 'तुमच्या दोष प्रकृतीनुसार संतुलित आहाराने मन, शरीर आणि आत्म्याचा समतोल साधा',
          explore: 'वैशिष्ट्ये पाहा'
        },
        how: { title: 'हे कसे कार्य करते' },
        cta: { startJourney: 'तुमची यात्रा सुरू करा' },
        features: {
          prakriti: { title: 'माझी प्रकृती', desc: 'तुमची अद्वितीय मन-शरीर प्रकृती जाणून घ्या.' },
          generate: { title: 'योजना तयार करा', desc: 'तुमच्या उद्दिष्टांवर आधारित नवी आहार योजना तयार करा.' },
          plans: { title: 'माझ्या आहार योजना', desc: 'जतन केलेल्या आणि पूर्वीच्या योजना पहा.' },
          herbs: { title: 'औषधी वनस्पती', desc: 'आयुर्वेदिक वनस्पती आणि त्यांचे फायदे जाणून घ्या.' },
          learn: { title: 'आयुर्वेद शिका', desc: 'आयुर्वेदिक तत्त्वांचे ज्ञान वाढवा.' },
          advisory: { title: 'स्मार्ट सल्ला', desc: 'तुमच्या आरोग्य प्रवासासाठी AI आधारित सल्ला.' }
        },
        footer: { copy: '© 2024 AyurBalance. सर्व हक्क राखीव.' }
      }
    },
    gu: {
      translation: {
        nav: { learn: 'આયુર્વેદ શીખો' },
        hero: {
          badge: '✨ પ્રાચીન જ્ઞાન, આધુનિક વિજ્ઞાન',
          title: 'તમારી વ્યક્તિગત<br/>આયુર્વેદિક આહાર યોજના મેળવો',
          subtitle: 'તમારી દોષ પ્રકૃતિ પ્રમાણેના આહારથી મન-શરીર-આત્માનો સંતુલન જાળો',
          explore: 'વિશેષતાઓ જુઓ'
        },
        how: { title: 'તે કેવી રીતે કાર્ય કરે છે' },
        cta: { startJourney: 'તમારી સફર શરૂ કરો' },
        features: {
          prakriti: { title: 'મારી પ્રકૃતિ', desc: 'તમારી અનોખી મન-શરીર રચના જાણો.' },
          generate: { title: 'યોજના બનાવો', desc: 'તમારા લક્ષ્યો પર આધારિત નવી આહાર યોજના બનાવો.' },
          plans: { title: 'મારી આહાર યોજનાઓ', desc: 'તમારી સાચવેલી અને જૂની યોજનાઓ જુઓ.' },
          herbs: { title: 'જડીબુટ્ટીઓ શોધો', desc: 'આયુર્વેદિક જડીબુટ્ટીઓ અને તેમના લાભો વિશે જાણો.' },
          learn: { title: 'આયુર્વેદ શીખો', desc: 'આયુર્વેદિક સિદ્ધાંતોનું જ્ઞાન વધારશો.' },
          advisory: { title: 'સ્માર્ટ સલાહ', desc: 'તમારી આરોગ્ય સફર માટે AI આધારિત સલાહ મેળવો.' }
        },
        footer: { copy: '© 2024 AyurBalance. સર્વ અધિકારો અનામત.' }
      }
    },
    bn: {
      translation: {
        nav: { learn: 'আয়ুর্বেদ শিখুন' },
        hero: {
          badge: '✨ প্রাচীন জ্ঞান, আধুনিক বিজ্ঞান',
          title: 'আপনার ব্যক্তিগত<br/>আয়ুর্বেদিক ডায়েট প্ল্যান পান',
          subtitle: 'আপনার দোষা অনুযায়ী খাদ্যাভ্যাসে মন-শরীর-আত্মার সমন্বয় আনুন',
          explore: 'বৈশিষ্ট্য দেখুন'
        },
        how: { title: 'কাজ করে কীভাবে' },
        cta: { startJourney: 'আপনার যাত্রা শুরু করুন' },
        features: {
          prakriti: { title: 'আমার প্রকৃতি', desc: 'আপনার অনন্য মন-শরীর গঠন জানুন।' },
          generate: { title: 'পরিকল্পনা তৈরি করুন', desc: 'আপনার লক্ষ্য অনুযায়ী নতুন ডায়েট পরিকল্পনা করুন।' },
          plans: { title: 'আমার ডায়েট পরিকল্পনা', desc: 'সংরক্ষিত ও পূর্ববর্তী পরিকল্পনা দেখুন।' },
          herbs: { title: 'ভেষজ জানুন', desc: 'আয়ুর্বেদিক ভেষজ ও তাদের উপকারিতা জানুন।' },
          learn: { title: 'আয়ুর্বেদ শিখুন', desc: 'আয়ুর্বেদের নীতিমালা সম্পর্কে জ্ঞান বাড়ান।' },
          advisory: { title: 'স্মার্ট পরামর্শ', desc: 'আপনার সুস্থতার জন্য AI ভিত্তিক পরামর্শ।' }
        },
        footer: { copy: '© 2024 AyurBalance. সর্বস্বত্ব সংরক্ষিত।' }
      }
    },
    ta: {
      translation: {
        nav: { learn: 'ஆயுர்வேதம் கற்போம்' },
        hero: {
          badge: '✨ பாரம்பரிய ஞானம், நவீன அறிவியல்',
          title: 'உங்கள் தனிப்பட்ட<br/>ஆயுர்வேத உணவு திட்டம் பெறுங்கள்',
          subtitle: 'உங்கள் தோஷத்திற்கு ஏற்ப உணவால் மனம்-உடல்-ஆன்மா சமநிலை',
          explore: 'அம்சங்களைப் பாருங்கள்'
        },
        how: { title: 'இது எப்படி செயல்படுகிறது' },
        cta: { startJourney: 'உங்கள் பயணத்தை தொடங்குங்கள்' },
        features: {
          prakriti: { title: 'என் பிரகிருதி', desc: 'உங்கள் தனித்துவமான மன-உடல் அமைப்பை அறியுங்கள்.' },
          generate: { title: 'திட்டம் உருவாக்கு', desc: 'உங்கள் இலக்குகளின் அடிப்படையில் புதிய திட்டம்.' },
          plans: { title: 'என் உணவு திட்டங்கள்', desc: 'சேமித்த மற்றும் முந்தைய திட்டங்கள்.' },
          herbs: { title: 'மூலிகைகள்', desc: 'ஆயுர்வேத மூலிகைகள் மற்றும் நன்மைகள்.' },
          learn: { title: 'ஆயுர்வேதம் கற்போம்', desc: 'ஆயுர்வேதக் கோட்பாடுகளை விரிவாக அறியவும்.' },
          advisory: { title: 'ஸ்மார்ட் ஆலோசனை', desc: 'உங்கள் நலனுக்கான AI ஆலோசனை.' }
        },
        footer: { copy: '© 2024 AyurBalance. அனைத்து உரிமைகளும் பாதுகாக்கப்பட்டவை.' }
      }
    }
  };

  if (!window.i18next) {
    console.warn('i18next not found');
    return;
  }

  window.i18next.init({ lng: defaultLng, fallbackLng: 'en', resources })
    .then(() => {
      translatePage();
    })
  ...
  const names = { en: 'English', hi: 'हिन्दी', mr: 'मराठी', gu: 'ગુજરાતી', bn: 'বাংলা', ta: 'தமிழ்' };

  if (label) label.textContent = names[defaultLng] || 'English';
  ...
  menu?.querySelectorAll('.lang-option').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      window.i18next.changeLanguage(lng).then(() => {
        ...
        if (label) label.textContent = names[lng] || lng;
        translatePage();
        closeMenu();
      });
    });
...
const quizQuestions = [
...

  {
    ...
    vata: { emoji: '🌬️', title: 'Vata', desc: 'Creative, energetic, and quick-thinking individuals governed by the elements of Air and Space. You thrive on movement and change, with a natural enthusiasm for life. Focus on warm, grounding foods, maintain regular routines, and practice calming activities like yoga and meditation.' },
    pitta: { emoji: '🔥', title: 'Pitta', desc: 'Passionate, intelligent, and driven individuals governed by Fire and Water. You have a sharp mind and strong digestion, with natural leadership qualities. Focus on cooling, calming foods, avoid excessive heat, and practice patience and compassion.' },
    kapha: { emoji: '🌿', title: 'Kapha', desc: 'Stable, compassionate, and strong individuals governed by Earth and Water. You have natural endurance and a calm demeanor, with excellent physical strength. Focus on light, warming foods, stay active, and embrace change and stimulation.' }
  },
...
function showAnswerReview() {
  ...
  const reviewHTML = `
    ...
    return `
      ...
      <div class='bg-ayur-lightBeige rounded-xl p-4 mb-5 border border-ayur-beige text-center'>
  ...
  content.innerHTML = `
    <div class='flex flex-col items-center justify-center py-12 space-y-4'>
      ...
      <div class='text-center'>
        <p class='text-lg font-semibold text-primary mb-2'>Analyzing Your Constitution</p>
 ...
 const result = await window.AyurAPI.calculatePrakriti(answers);    
    ...
    resultDiv.innerHTML = `
      <div class='text-center animate-fade-in'>
        <div class='inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br from-primary to-secondary mb-4 animate-pulse-soft'>
        ...
        <p class='font-serif text-5xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent mb-5'>
    ...
    document.getElementById('saveDosha').addEventListener('click', () => {
      ...
      // Store primary dosha for plan generation (radio buttons only support single)
      setDemoProfile(dosha, []);
      log(`Dosha ${dosha} saved to profile`);
      prakritiModal.classList.add('hidden');
      planModal.classList.remove('hidden');
      // Load saved profile into plan form
      loadSavedProfileIntoPlanForm();
    });
...
document.getElementById('btnGeneratePlan').addEventListener('click', () => {
  // Load saved profile and pre-populate form
  loadSavedProfileIntoPlanForm();
  planModal.classList.remove('hidden');
  log('Opened Generate Plan');
});

function loadSavedProfileIntoPlanForm() {
  const savedProfile = JSON.parse(localStorage.getItem('profile') || '{}');
  
  if (savedProfile.dosha_result) {
    // Pre-select dosha radio
    const doshaRadio = document.querySelector(`input[name="planDoshaRadio"][value="${savedProfile.dosha_result}"]`);
    if (doshaRadio) doshaRadio.checked = true;
  }
  
  if (savedProfile.health_goals && savedProfile.health_goals.length > 0) {
    // Pre-check goal checkboxes
    savedProfile.health_goals.forEach(goal => {
      const checkbox = document.querySelector(`input[name="planGoalCheck"][value="${goal}"]`);
      if (checkbox) checkbox.checked = true;
    });
  }
  
  if (savedProfile.age_years) {
    const ageInput = document.getElementById('planAge');
    if (ageInput) ageInput.value = savedProfile.age_years;
  }
  
  if (savedProfile.weight_kg) {
    const weightInput = document.getElementById('planWeight');
    if (weightInput) weightInput.value = savedProfile.weight_kg;
  }
  
  log('Loaded saved profile into plan form');
}
  ...
  try {
    ...
    document.getElementById('planForm').innerHTML = `
      <div class='glass-effect rounded-xl p-6 border-2 border-red-200 dark:border-red-800 animate-fade-in'>
        ...
        <div class='bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4 mb-4 border border-yellow-200 dark:border-yellow-800'>
      ...
      if (allLoadedItems.length === 0) {
        resultsDiv.innerHTML = `
          <div class='text-center py-12'>
            ...
            <p class='text-sm text-gray-500 dark:text-gray-500 mb-4'>Try searching with different keywords</p>
          </div>
        `;
      } else {
        const foodsHTML = allLoadedItems.map((food, idx) => `
          <div class='glass-effect rounded-xl p-4 border border-border-light dark:border-border-dark hover:shadow-lg transition-all duration-300 animate-fade-in' style='animation-delay: ${(idx % 30) * 20}ms'>
            <div class='flex items-start gap-3'>
              <div class='w-12 h-12 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-white text-xl flex-shrink-0'>
                🌿
              </div>
              <div class='flex-1'>
...
// Dark Mode Toggle - Initialize after DOM is ready
...
// 6. COMMUNITY FORUM - Initialize after DOM ready
function initializeCommunityForum() {
  const btnViewAllDiscussions = document.getElementById('btnViewAllDiscussions');
  const btnStartDiscussion = document.getElementById('btnStartDiscussion');

  if (btnViewAllDiscussions) {
    btnViewAllDiscussions.addEventListener('click', () => {
      if (typeof communityModal !== 'undefined') {
    ...
    btnStartDiscussion.addEventListener('click', () => {
      if (!currentUser) {
        ...
        if (typeof communityModal !== 'undefined') {
          ...
          if (typeof renderForumList === 'function') renderForumList();
          setTimeout(() => {
            if (typeof showNewDiscussionForm === 'function') showNewDiscussionForm();
          }, 100);
        }
      }
    });
...
function initializeExpertConnect() {
  const btnConnectExpert = document.getElementById('btnConnectExpert');
  if (btnConnectExpert) {
    btnConnectExpert.addEventListener('click', () => {
    ...
    confirmDiv.innerHTML = `
      <div class='animate-fade-in'>
        <div class='text-center mb-6'>
          ...
          <h3 class='text-2xl font-bold text-primary mb-2'>Appointment Booked Successfully!</h3>
...
// Add dynamic animations for elements on scroll
...
window.addEventListener('load', () => {
  ...
  // Initialize feature card animations after DOM is loaded
...
const rightNowAdviceMap = {
  tired: {
    title: 'Feeling tired?',
    text: 'Take a 10-minute rest and sip warm ginger or tulsi tea. Keep meals light and grounding to support your energy.'
  },
  heavy: {
    title: 'Feeling heavy after lunch?',
    text: 'Try a gentle 10-minute walk before your next meal and choose lighter, more digestible foods to ease post-meal heaviness.'
  },
  irritated: {
    title: 'Feeling irritated?',
    text: 'Cool the system with a few deep breaths, a short walk in shade, and avoid overly spicy, fried, or heated foods for now.'
  },
  restless: {
    title: 'Feeling restless?',
    text: 'Ground yourself with a calming routine: breathe deeply, reduce screen time, and favor warm, nourishing foods with a steady rhythm.'
  },
  bloated: {
    title: 'Feeling bloated?',
    text: 'Pause eating for a moment, sip warm water slowly, and keep your next meal simple with cooked vegetables and light grains.'
  },
  fine: {
    title: 'Feeling fine today?',
    text: 'Keep your rhythm steady. A balanced meal, hydration, and light movement will help maintain the calm balance you already have.'
  }
};

function initializeRightNowMode() {
  const buttons = document.querySelectorAll('.feeling-btn');
  const advicePanel = document.getElementById('rightNowAdvice');
  const launcher = document.getElementById('assistantLauncher');
  const panel = document.getElementById('assistantPanel');
  const closeBtn = document.getElementById('assistantClose');

  if (!buttons.length || !advicePanel || !launcher || !panel) return;

  const setAdvice = (feeling, text, title) => {
    const selected = rightNowAdviceMap[feeling] || rightNowAdviceMap.heavy;
    const displayTitle = title || selected.title;
    const displayText = text || selected.text;

    buttons.forEach((button) => {
      const isActive = button.dataset.feeling === feeling;
    ...
    advicePanel.innerHTML = `
      <h4 class="text-lg font-bold text-text-light dark:text-text-dark">${displayTitle}</h4>
      <p class="mt-2 text-sm text-text-light/80 dark:text-text-dark/80 leading-relaxed">
        ${displayText}
      </p>
    `;
  };

  const fetchAiAdvice = async (feeling) => {
    const profile = JSON.parse(localStorage.getItem('profile') || '{}');
    const dosha = profile.dosha_result || 'balanced';
    const goals = Array.isArray(profile.health_goals) ? profile.health_goals.join(', ') : 'general wellness';
    const feelingLabel = rightNowAdviceMap[feeling]?.title || 'Feeling ' + feeling;

    const prompt = `I am feeling ${feeling}. My Ayurvedic prakriti/dosha is ${dosha}. My goals are ${goals}. Give me a practical Ayurvedic action I can take right now for my body and mind. Keep it short, specific, and actionable. Mention what to do immediately and why. Do not suggest allopathic medicine. Stay in general wellness guidance and keep it under 2-3 sentences.`;

    setAdvice(feeling, 'Checking your context and preparing a personalized action...', 'AyurBot is thinking...');
    ...
    try {
      ...
      if (reply) {
        ...
        setAdvice(feeling, replyText, feelingLabel);
        return;
      }

      throw new Error('No reply returned');
    } catch (error) {
      console.error('Right now AI advice failed:', error);
      const selected = rightNowAdviceMap[feeling] || rightNowAdviceMap.heavy;
      setAdvice(feeling, selected.text, selected.title);
    }
  };
  ...
  buttons.forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      await fetchAiAdvice(feeling);
    });
  });

  setAdvice('heavy', rightNowAdviceMap.heavy.text, rightNowAdviceMap.heavy.title);
}

// Medicine Alternatives Feature
class MedicineAlternatives {
  constructor() {
    this.medicineData = {
      'paracetamol': {
        ...
        alternatives: [
          {
            ...
            form: 'powder',
            description: 'Effective for fever reduction and general malaise'
          },
          {
            name: 'Mrityunjaya Rasa',
            form: 'tablet',
            description: 'Powerful fever reducer and immune booster'
          },
          {
            name: 'Godanti Bhasma',
            form: 'powder',
            description: 'Cooling effect, reduces fever and inflammation'
          },
          {
            name: 'Sitopaladi Churna',
            form: 'churnam',
            description: 'Natural antipyretic and pain reliever'
          }
        ]
      },
      'aspirin': {
        uses: 'pain relief, blood thinner, fever',
        alternatives: [
          {
            name: 'Saraswatarishta',
            form: 'liquid',
            description: 'Natural blood thinner and circulation enhancer'
          },
          {
            name: 'Dashamool Kwath',
            form: 'kadha',
          ...
          {
            name: 'Turmeric with warm milk',
            form: 'powder',
            description: 'Natural anti-inflammatory and pain relief'
          }
        ]
      },
      'ibuprofen': {
        uses: 'inflammation, pain relief, fever',
        alternatives: [
          {
            name: 'Mahayogaraj Guggulu',
            form: 'tablet',
            description: 'Powerful anti-inflammatory for joints and muscles'
          },
          {
            name: 'Boswellia (Shallaki)',
            form: 'capsule',
            description: 'Natural COX enzyme inhibitor, reduces inflammation'
          },
          {
            name: 'Rasna Saptak Kwath',
            form: 'kadha',
            description: 'Effective for inflammatory conditions and pain'
          },
          {
            name: 'Ginger-Turmeric paste',
            form: 'paste',
            description: 'Natural anti-inflammatory combination'
          }
        ]
      },
      'omeprazole': {
        ...
        alternatives: [
          {
            ...
            form: 'churnam',
            description: 'Balances stomach acid and reduces heartburn'
          },
          {
            name: 'Kamadudha Rasa',
            form: 'tablet',
            description: 'Cooling effect on stomach, reduces acidity'
          },
          {
            name: 'Sukumara Ghrita',
            form: 'ghee',
            description: 'Heals gastric ulcers and reduces acid production'
          },
          {
            name: 'Amla juice',
            form: 'juice',
            description: 'Natural antacid with cooling properties'
          }
        ]
      },
      'metformin': {
        uses: 'diabetes type 2, blood sugar control',
        alternatives: [
          {
            name: 'Madhumehantak Churna',
            form: 'churnam',
            description: 'Traditional formula for blood sugar management'
          },
          {
            name: 'Karela (Bitter Gourd) juice',
            form: 'juice',
            description: 'Natural insulin-like properties'
          },
          {
            name: 'Vijaysar (Pterocarpus)',
            form: 'powder',
            description: 'Effective in glucose metabolism regulation'
          },
          {
            name: 'Gymnema Sylvestre',
            form: 'capsule',
            description: 'Sugar destroyer herb, reduces glucose absorption'
          }
        ]
      },
      'losartan': {
        ...
        alternatives: [
          {
            name: 'Arjunarishta',
            form: 'liquid',
            description: 'Cardio-protective, helps regulate blood pressure'
          },
          {
            name: 'Saraswatarishta',
            form: 'liquid',
            description: 'Calms nervous system, supports heart health'
          },
          {
            name: 'Punarnava Mandur',
            form: 'tablet',
            description: 'Diuretic properties, helps reduce BP naturally'
          },
          {
            name: 'Garlic with honey',
            form: 'paste',
            description: 'Natural ACE inhibitor, cardiovascular support'
          }
        ]
      },
      'atorvastatin': {
        ...
        alternatives: [
          {
            ...
            form: 'powder',
            description: 'Natural cholesterol reducer, heart protective'
          },
          {
            name: 'Medohar Guggulu',
            form: 'tablet',
            description: 'Traditional lipid metabolism enhancer'
          },
          {
            name: 'Triphala Churna',
            form: 'churnam',
            description: 'Detoxifies and improves metabolism'
          },
          {
            name: 'Lemon-honey water',
            form: 'drink',
            description: 'Natural fat metabolizer and cholesterol reducer'
          }
        ]
      }
    };
  ...
  initializeEventListeners() {
    // Open modal button
    const medicineBtn = document.getElementById('medicineAltBtn');
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && medicineModal.style.display === 'flex') {
        medicineModal.style.display = 'none';
        this.clearResults();
      }
    });
  ...
  async searchMedicine() {
    ...
    if (!medicineName) {
      this.showError('Please enter a medicine name');
      return;
    }
    ...
    if (searchBtn) {
      ...
      searchBtn.innerHTML = '<span class="material-symbols-outlined rotating">progress_activity</span>';
    }
    ...
    resultsContainer.innerHTML = `
      <div class="medicine-loading">
        ...
        <p>Searching for Ayurvedic alternatives...</p>
        <p class="text-sm opacity-70">Consulting AI Ayurvedic expert...</p>
      </div>
    `;

    try {
      // Call the API to get medicine alternatives from Hugging Face
      const data = await AyurAPI.getMedicineAlternative(medicineName);
      
      if (data.alternatives && data.alternatives.length > 0) {
        this.displayAlternativesFromAI(medicineName, data);
      } else {
        this.showNotFound(medicineName);
      }
    } catch (error) {
      console.error('Error fetching medicine alternatives:', error);
      this.showError(`Failed to fetch alternatives: ${error.message}`);
    } finally {
      // Re-enable button
      if (searchBtn) {
  ...
  displayAlternatives(medicineName, medicineInfo) {
    const resultsContainer = document.getElementById('medicineResults');

    let html = `
      <div class="medicine-search-result">
        <h3 style="color: #556B2F; font-size: 1.1rem; font-weight: 600; margin-bottom: 0.5rem; text-transform: capitalize;">
          ${medicineName}
        </h3>
        <p style="color: #6b7280; font-size: 0.875rem; margin-bottom: 1.5rem;">
          Common uses: ${medicineInfo.uses}
        </p>
        <h4 style="color: #374151; font-size: 1rem; font-weight: 600; margin-bottom: 1rem;">
          🌿 Ayurvedic Alternatives:
        </h4>
      </div>
    `;

    medicineInfo.alternatives.forEach((alt, index) => {
      html += `
        <div class="medicine-alternative" style="animation-delay: ${index * 0.1}s;">
          <h4>${alt.name}</h4>
          <div class="medicine-form-info">${alt.form}</div>
          <p class="medicine-description">${alt.description}</p>
        </div>
      `;
    });
    ...
    // Add entrance animation to alternatives
    const alternatives = resultsContainer.querySelectorAll('.medicine-alternative');
    alternatives.forEach((alt, index) => {
      alt.style.opacity = '0';
      alt.style.transform = 'translateY(20px)';
      setTimeout(() => {
        alt.style.transition = 'all 0.4s ease';
        alt.style.opacity = '1';
  ...
  displayAlternativesFromAI(medicineName, data) {
    const resultsContainer = document.getElementById('medicineResults');

    let html = `
      <div class="medicine-result-header">
        <h3 style="color: #556B2F; font-size: 1.25rem; font-weight: 600; margin-bottom: 0.5rem;">
          🌿 Ayurvedic Alternatives for <strong style="text-transform: capitalize;">${medicineName}</strong>
        </h3>
        <p style="color: #6b7280; font-size: 0.875rem; margin-bottom: 1rem;">
          AI-Generated recommendations by AyurBot
        </p>
      </div>
      <div class="medicine-alternatives">
    `;
      ...
      html += `
        <div class="medicine-card" style="animation-delay: ${index * 0.1}s;">
          ...
          <div class="medicine-card-body">
            ${alt.description ? `<p class="medicine-description">${alt.description}</p>` : ''}
            ...
            ${alt.precautions ? `
              <div class="medicine-precautions">
    ...
    html += `
      ...
      ${data.disclaimer ? `
        <div class="medicine-disclaimer-inline">
    ...
    // Add entrance animation to alternatives
    const alternatives = resultsContainer.querySelectorAll('.medicine-card');
    alternatives.forEach((alt, index) => {
      alt.style.opacity = '0';
      alt.style.transform = 'translateY(20px)';
      setTimeout(() => {
  ...
  showNotFound(medicineName) {
    const resultsContainer = document.getElementById('medicineResults');
    resultsContainer.innerHTML = `
      <div class="medicine-error">
        <h4 style="margin-bottom: 0.5rem;">❌ Medicine Not Found</h4>
        <p style="margin: 0;">No trusted Ayurvedic data found for "${medicineName}".</p>
        <p style="margin: 0.5rem 0 0 0; font-size: 0.75rem;">Try searching for: paracetamol, aspirin, ibuprofen, omeprazole, metformin, losartan, or atorvastatin.</p>
      </div>
    `;
  }
  ...
  showError(message) {
    ...
    resultsContainer.innerHTML = `
      <div class="medicine-error">
        <h4 style="margin-bottom: 0.5rem;">⚠️ Error</h4>
  ...
  clearResults() {
    ...
    resultsContainer.innerHTML = `
      <div class="medicine-welcome">
        <div class="medicine-welcome-icon">🌿</div>
        <p class="medicine-welcome-text">Enter an allopathic medicine name to find Ayurvedic alternatives</p>
      </div>
    `;
...
// Initialize Medicine Alternatives feature
document.addEventListener('DOMContentLoaded', () => {
  // Initialize all features that require DOM access
  initializeApp();
});

// Main initialization function
function initializeApp() {
  console.log('🚀 Initializing AyurBalance Application...');
  ...
  try {
    ...
    initializeExpertConnect();
    initializeRightNowMode();

    // Initialize medicine alternatives
    new MedicineAlternatives();
    window.medicineAlternativesInitialized = true;

    console.log('✅ AyurBalance Application initialized successfully');
  } catch (error) {
...
window.addEventListener('load', () => {
  if (!window.medicineAlternativesInitialized) {
    try {
      initializeApp();
      console.log('Medicine Alternatives fallback initialization successful');
    } catch (error) {
      console.error('Medicine Alternatives fallback failed:', error);
    }
  }
});
